...
def train(train_loader, train_table, model, model_bert, opt, bert_config, tokenizer,
    ...
    for batch_index, batch_data in enumerate(train_loader):
        ...
        loss = Loss_selectwhere_startend_v2(score_select_column, score_select_agg, score_where_number, score_where_column,
                          score_where_op, score_where_value, gt_select_column, gt_select_agg, gt_wherenumber, gt_wherecolumn, g_wo, gt_wherevalueindex)
        ...
        pr_sc, pr_sa, pr_wn, pr_wc, pr_wo, pr_wvi = pred_selectwhere_startend(score_select_column, score_select_agg,
                                                                                  score_where_number,
                                                                                  score_where_column, score_where_op,
                                                                                  score_where_value, )
        ...
        while batch_have_good_reward == False:
            ...
            pred_wherevalue_str_random_v2, pred_wherevalue_str_bert_random_v2 = convert_pred_wvi_to_string(pred_wherevalueindex_random_v2, question_token,
                                                                                       question_token_bert,
                                                                                       berttoken_to_token_index, question)

            # pred_wherecolumn_sorted_random_v2 = sort_pred_wherecolumn(wherecolumn_random_v2, gt_wherecolumn)

            random_sql_int_v2 = generate_sql_i_v2(pred_selectcolumn_random, pred_selectagg_random, wherenumber_random_v2,
                                                  wherecolumn_random_v2, whereop_random_v2, pred_wherevalue_str_random_v2, question)
            ...
            for i in range(len(batch_data)):
                ...
                pred_answer_column = engine.execute(table[i]['id'], selectcolumn_random_v2[i], selectagg_random_v2[i], tmp_conds)
                # answer in
        ...
        RL_loss_select_agg = torch.mean(-torch.log(torch.sum(
            torch.softmax(score_select_agg, dim=-1) * torch.tensor(onehot_action_batch_selectagg,
                                                                      dtype=torch.float).to(device), dim=-1)
        ) * torch.tensor(batch_reward_all, dtype=torch.float).to(device))
        ...
        tmp_action = torch.tensor(action_wherevalue, dtype=torch.float).to(device)
        ...
        ) * torch.unsqueeze(torch.unsqueeze(torch.tensor(batch_reward_all, dtype=torch.float),
                        dim=-1),dim=-1).to(device))
        ...
        tmp_action_op = torch.tensor(action_whereop, dtype=torch.float).to(device)
        RL_loss_where_op = torch.mean(
            -torch.log(torch.sum(
                torch.softmax(score_where_op, dim=-1) * tmp_action_op, dim=-1)
            ) * torch.unsqueeze(torch.tensor(batch_reward_all, dtype=torch.float),dim=-1).to(device))
