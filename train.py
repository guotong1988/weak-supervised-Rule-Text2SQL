...
def get_data(path_wikisql, args):
    train_data, train_table, dev_data, dev_table, _, _ = load_wikisql_v2(path_wikisql, args.toy_model, args.toy_size, no_w2i=True, no_hs_tok=True)
...
def train(train_loader, train_table, model, model_bert, opt, bert_config, tokenizer,
    ...
    count_lx = 0  # of logical form acc
    count_x = 0   # of execution acc
    ...
    for iB, t in enumerate(train_loader):
        ...
        gt_select_column, gt_select_agg, g_wn, g_wc, g_wo, g_wv = get_gt(sql_i)
        # get ground truth where-value index under CoreNLP tokenization scheme. It's done already on trainset.
        g_wvi_corenlp = get_gt_wherevalueindex_corenlp(t)
        ...
        try:
            #
            g_wvi = get_gt_wherevalueindex_bert_from_gt_wherevalueindex_corenlp(t_to_tt_idx, g_wvi_corenlp)
        except:
        ...
        # score
        score_select_column, score_select_agg, s_wn, s_wc,\
        s_wo, s_wv,\
            _,_,_,_,_,_, = model(emb_question, len_question, emb_header, len_header_token, number_header,
                           g_sc=gt_select_column, g_sa=gt_select_agg, g_wn=g_wn,
                           g_wc=g_wc, g_wvi=g_wvi)

        # Calculate loss & step
        loss = Loss_sw_se(score_select_column, score_select_agg, s_wn, s_wc,
                          s_wo, s_wv, gt_select_column, gt_select_agg, g_wn, g_wc, g_wo, g_wvi)
        ...
        # Prediction
        pr_sc, pr_sa, pr_wn, pr_wc, pr_wo, pr_wvi = pred_selectwhere_startend(score_select_column, score_select_agg, s_wn, s_wc, s_wo, s_wv, )
        pr_wv_str, pr_wv_str_wp = convert_pred_wvi_to_string(pr_wvi, nlu_t, nlu_tt, tt_to_t_idx, nlu)
        ...
        pr_wc_sorted = sort_pred_wherecolumn(pr_wc, g_wc)
        ...
        count_wvi1_list, count_wv1_list = get_count_sw_list(gt_select_column, gt_select_agg, g_wn, g_wc, g_wo, g_wvi,
        ...
        count_x1_list, g_ans, pr_ans = get_count_x_list(engine, tb, gt_select_column, gt_select_agg, sql_i, pr_sc, pr_sa, pr_sql_i)
        ...
        count_lx += sum(count_lx1_list)
        count_x += sum(count_x1_list)
...
def report_detail(hds, nlu,
    ...
    count_tot, count, count_sc, count_sa, count_wn, count_wc, count_wo, count_wv, count_wvi, count_lx, count_x = current_count

    print(f'count = {count} / {count_tot} ===============================')
    ...
    print(f'acc_lx = {count_lx/count:.3f}, acc_x = {count_x/count:.3f}\n',
          f'acc_sc = {count_sc/count:.3f}, acc_sa = {count_sa/count:.3f}, acc_wn = {count_wn/count:.3f}\n',
          f'acc_wc = {count_wc/count:.3f}, acc_wo = {count_wo/count:.3f}, acc_wv = {count_wv/count:.3f}')
    print(f'===============================')

def test(data_loader, data_table, model, model_bert, bert_config, tokenizer,
         max_seq_length,
         num_target_layers, detail=False, st_pos=0, count_tot=1, EG=False, beam_size=4,
    ...
    for iB, t in enumerate(data_loader):
        ...
        g_sc, g_sa, g_wn, g_wc, g_wo, g_wv = get_gt(sql_i)
        g_wvi_corenlp = get_gt_wherevalueindex_corenlp(t)
        ...
        try:
            g_wvi = get_gt_wherevalueindex_bert_from_gt_wherevalueindex_corenlp(t_to_tt_idx, g_wvi_corenlp)
            g_wv_str, g_wv_str_wp = convert_pred_wvi_to_string(g_wvi, nlu_t, nlu_tt, tt_to_t_idx, nlu)
        ...
        if not EG:
            ...
            _,_,_,_,_,_\
                = model(wemb_n, l_n, wemb_h, l_hpu, l_hs)

            # get loss & step
            loss = Loss_sw_se(s_sc, s_sa, s_wn, s_wc, s_wo, s_wv, g_sc, g_sa, g_wn, g_wc, g_wo, g_wvi)

            # prediction
            pr_sc, pr_sa, pr_wn, pr_wc, pr_wo, pr_wvi = pred_selectwhere_startend(s_sc, s_sa, s_wn, s_wc, s_wo, s_wv, )
            pr_wv_str, pr_wv_str_wp = convert_pred_wvi_to_string(pr_wvi, nlu_t, nlu_tt, tt_to_t_idx, nlu)
        ...
        count_sc1_list, count_sa1_list, count_wn1_list, \
        count_wc1_list, count_wo1_list, \
        count_wvi1_list, count_wv1_list = get_count_sw_list(g_sc, g_sa,g_wn, g_wc,g_wo, g_wvi,
                                                                   pr_sc, pr_sa, pr_wn, pr_wc, pr_wo, pr_wvi,
                                                                   sql_i, pr_sql_i,
                                                                   mode='test')
        ...
        # Execution accuracy test.
        count_x1_list, g_ans, pr_ans = get_count_x_list(engine, tb, g_sc, g_sa, sql_i, pr_sc, pr_sa, pr_sql_i)
        ...
        current_count = [count_tot, count, count_sc, count_sa, count_wn, count_wc, count_wo, count_wv, count_wvi, count_lx, count_x]
        count_list1 = [count_sc1_list, count_sa1_list, count_wn1_list, count_wc1_list, count_wo1_list, count_wv1_list, count_lx1_list,
                     count_x1_list]
    ...
    acc_wc = count_wc / count
    acc_wo = count_wo / count
    acc_wvi = count_wvi / count
    acc_wv = count_wv / count
    acc_lx = count_lx / count
    acc_x = count_x / count

    acc = [ave_loss, acc_sc, acc_sa, acc_wn, acc_wc, acc_wo, acc_wvi, acc_wv, acc_lx, acc_x]
    return acc, results, count_list
