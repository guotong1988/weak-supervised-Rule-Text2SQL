...
def test(data_loader, data_table, model, model_bert, bert_config, tokenizer,
    ...
    for iB, t in enumerate(data_loader):
        ...
        count_list_batch = [count_sc1_list, count_sa1_list, count_wn1_list, count_wc1_list, count_wo1_list, count_wv1_list, count_lx1_list,
                     count_x1_list]
        count_list.append(count_list_batch)
        # report
        if detail:
            report_detail(hds, nlu,
                          g_sc, g_sa, g_wn, g_wc, g_wo, g_wv, g_wv_str, g_sql_q, g_ans,
                          pr_sc, pr_sa, pr_wn, pr_wc, pr_wo, pr_wv_str, pr_sql_q, pr_ans,
                          count_list_batch, current_count)
        data_batch = []
    ...
    acc = [ave_loss, acc_sc, acc_sa, acc_wn, acc_wc, acc_wo, acc_wvi, acc_wv, acc_lx, acc_x]
    return acc, results, count_list, p_list, data_list
...
if __name__ == '__main__':
    ...
    for epoch in range(args.tepoch):
        ...
        # check DEV
        with torch.no_grad():
            acc_dev, results_dev, count_list_dev, p_list_dev, data_list_dev = test(dev_loader,
                                                dev_table,
                                                model,
                                                model_bert,
                                                bert_config,
